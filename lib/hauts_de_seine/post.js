...
function generateXML(appointment, folder) {
  console.log('appointment', appointment)
  const templatePath = path.join(folder, 'lib/hauts_de_seine/templates/bookRequest.xml.hbs')
  const bookRequestTemplate = Handlebars.compile(fs.readFileSync(templatePath, 'utf-8'))

  return bookRequestTemplate(appointment)
}

function book(appointment) {
  const body = generateXML(appointment)
  const options = {
